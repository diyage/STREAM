...
class SelfLstm(nn.Module):
    def __init__(self, num_hidden, num_layers=1, just_used_last_time_stamp: bool = True):
        super().__init__()
        self.self_lstm = nn.LSTM(input_size=num_hidden, hidden_size=num_hidden, num_layers=num_layers, batch_first=True)
        self.just_used_last_time_stamp = just_used_last_time_stamp

    def forward(self, hs, ):
        output, _ = self.self_lstm(hs)
        if self.just_used_last_time_stamp:
            return output[:, -1]
        else:
            return output
...
class EdgeResidualPredict(torch.nn.Module):
    ...
    def forward(self, x_stream_flow: torch.Tensor, edge_features: torch.Tensor):
        ...
        features = torch.cat([x_stream_flow, edge_features], dim=-1)  # [b, n, t, 1 + edge_feature_dim]

        features_embed = self.embed_layer(features)  # [b, n, t, mid]
